...
void Framework::Run(GameBase& game)
{
	...
	while (!done)
	{
		...
		RenderText(std::format("Accuracy:  {:.2f} (hit/shot: {}/{})", game.tank && game.tank->numShots != 0 ? ((float)game.tank->score / game.tank->numShots) : 0.0f, game.tank->score, game.tank->numShots), { 0, 180 });
